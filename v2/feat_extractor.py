...
from v2 import network_architectures as netark
from v2 import extractor as exm
...
pre_model_path = 'checkpoints/mx-h64-1024_0d3-1.17.pkl'
...
def extract_features(y, sr, srate):
    if len(y.shape) > 1:
...
def extract_from_file(filename, srate=44100):
    try:
        y, sr = lib.load(filename, sr=None)
    except:
        raise IOError('Give me an audio  file which I can read!!')

    return extract_features(y, sr, srate)


def extract_from_data(data, sr, srate=44100):
    return extract_features(data, sr, srate)

# pred = extract_from_file('files/1-977-A-39.wav')
